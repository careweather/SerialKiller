...
from PyQt5.QtCore import (QByteArrayMatcher, QFile, QObject, QSaveFile,
                          QThread, QTimer, flush, pyqtSignal)
from PyQt5.QtGui import QFont, QIntValidator, QPalette, QTextCursor
# library imports
    ...
    def connect(self, **kwargs) -> bool:
        ...
        if '-b' in kwargs:
            ...
            if baud not in self.get_combobox_items(self.ui.comboBox_baud):
                self.debug_text(
                    f"ERR: BAUD RATE {baud} INVALID", color=COLOR_RED)
                return
        ...
        if not self.serial.connect(port = port, baud = baud, xonxoff=xonxoff, rtscts=rtscts, dsrdtr=dsrdtr, parity=parity):
            ...
            self.add_text(
                f"ERR: {port} COULD NOT CONNECT \n{self.serial.error_message}", type=TYPE_ERROR)
            return False
        ...
        self.ui.pushButton_send.setStyleSheet(STYLE_SHEET_BUTTON_ACTIVE)
        ...
        self.serial.incoming.connect(self.add_text)
        self.serial.disconnect_signal.connect(self.serial_error)
        self.serial_thread.setTerminationEnabled(True)
        self.serial_thread.start()
        return True

    def connect_clicked(self, source=None):
        if self.serial.connected:
            self.disconnect()
        else:
            self.connect()
        ...
        try:
            ...
            for checkbox in self.save_checkboxes:
                ...
                if checkbox.objectName() in self.current_settings:
                    checkbox.setChecked(
                        self.current_settings[checkbox.objectName()])
                else:
...
########################################################################
    ...
    def start_script(self, file_name: str = None, delay=None, arg_str=""):
        if delay == None:
            delay = get_number(self.ui.lineEdit_delay.text(), int, 200)
        text: str = None
        if not file_name:
            ...
            if text == None:
                self.add_text("Script '", file_name,
                              "' NOT FOUND\n", type=TYPE_ERROR)
        if not text:
            return
        ...
        if '-o' in kwargs:
            if kwargs['-o']:
                ...
                if not os.path.exists(log_file):
                    self.add_text(
                        f"Log file {log_file} not found", type=TYPE_ERROR)
                    return
            else:
